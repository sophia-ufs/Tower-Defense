...
// analisar se um defensor consegue atingir um invasor
// cada defensor tem sey alcance definido
const alcança = (def, inv) => {

    //d -> quadrado da distancia entre o defensor e o invasor
    const d = dist(def.x, def.y, inv.x, inv.y)

    //se d for menor ou igual ao quadrado do alcance do defensor, ent ele consegue
    //atingir o invasor
...
// converter o valor que representa a direção do movimento de um invasor
...
// se sim, a direção será mudado, se não permanece a msm
const updtDir = (x, y, dir) => {
    const aux = achar({x:x, y:y}, mapaUm)[0]
    if(indef(aux)) return dir
    else return aux.nd
}
...
const updtPos = (inv) => {
    ...
    const mov = inv.map((curr) => {
        ...
        const aux = invasor(curr.nome, curr.vida, curr.ataque, 
    ...
    // inv_tela é a lista filtrando os invasores que já ultrapassaram o mapa
    const inv_tela = mov.filter((inv) => inv.y < 560)

    return {inv : inv_tela, perda : perda }
}

//função recursiva pque retorna os invasores com suas vidas atualizadas após o ataque dos defensores
// e quanto de moedas se ganhou com essa onda de ataques
...
// função recursiva que representa a horda
/*
...
const horda = async (inv_curr, vida, moedas, inv_fora, def) => {
    ...
    if(inv_curr.length == 0 || vida <= 0){
        // caso base: ou não tem nenhum invasor na tela, ou o jogador perdeu todas suas vidas
